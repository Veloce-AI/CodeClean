...
2. Name it `CodeClean`, set it to **Public**, leave everything else unchecked
3. Click **Create repository**

### Step 2 — Push the code

Open a terminal in the `C:\VsCode\CodeClean` folder:

```bash
git init
git add .
git commit -m "Initial release — CodeClean v1.0.0"
git branch -M main
git remote add origin https://github.com/Veloce-AI/CodeClean.git
...
Go to your repo on GitHub → **Add file** → **Create new file** → name it `LICENSE` → click **Choose a license template** → select **MIT** → fill in your name → commit.

### Step 4 — Set up GitHub Pages (optional — lets people open it directly in the browser)

1. GitHub repo → **Settings** → **Pages**
2. Source: **Deploy from a branch** → branch: `main` → folder: `/ (root)`
3. Click **Save**
4. Your tool is now live at `https://YOUR-USERNAME.github.io/CodeClean/`

---

## Deploying to VS Code Marketplace

### Prerequisites

- A Microsoft account (free) — [signup here](https://account.microsoft.com)
...
4. Choose a publisher ID (e.g. `VeloceAI`) — this goes in `package.json` as `"publisher"`
5. Update `package.json` → set `"publisher": "your-publisher-id"`
...
5. Expiration: 1 year
6. Scopes: **Custom** → tick **Marketplace → Manage**
7. Click **Create** → **copy the token immediately** (shown only once)

### Step 3 — Install vsce and log in
...
```bash
cd C:\VsCode\CodeClean
vsce package
# Creates: codeclean-1.0.0.vsix
```

> If you get a warning about `logo.png` missing from `.vscodeignore`, make sure it's listed as included (not ignored).

### Step 5 — Test locally before publishing

```bash
# Install the .vsix in your own VS Code to verify it works
code --install-extension codeclean-1.0.0.vsix
```

Then open VS Code → look for the CodeClean icon in the Activity Bar → click it → confirm everything works.
...
`https://marketplace.visualstudio.com/items?itemName=your-publisher-id.codeclean`

It typically takes **5–10 minutes** to appear in search results.

### Step 7 — Update the README badges

Once published, replace `Veloce-AI` in the badge URLs with your actual publisher ID:

```
https://img.shields.io/visual-studio-marketplace/v/YOUR-PUBLISHER-ID.codeclean
https://img.shields.io/visual-studio-marketplace/i/YOUR-PUBLISHER-ID.codeclean
```
